...
#[derive(CodamaType)]
pub struct VestingRecipientAddedEvent {
...
impl EventDiscriminator for VestingRecipientAddedEvent {
    const DISCRIMINATOR: u8 = EventDiscriminators::VestingRecipientAdded as u8;
}

impl EventSerialize for VestingRecipientAddedEvent {
    #[inline(always)]
...
impl VestingRecipientAddedEvent {
...
mod tests {
    ...
    #[test]
    fn test_vesting_recipient_added_event_new() {
        let distribution = Address::new_from_array([1u8; 32]);
        let recipient = Address::new_from_array([2u8; 32]);

        let event = VestingRecipientAddedEvent::new(distribution, recipient, 1000, 0, 100, 200);
    ...
    #[test]
    fn test_vesting_recipient_added_event_to_bytes_inner() {
        let distribution = Address::new_from_array([1u8; 32]);
        let recipient = Address::new_from_array([2u8; 32]);
        let event = VestingRecipientAddedEvent::new(distribution, recipient, 5000, 0, 100, 200);

        let bytes = event.to_bytes_inner();
        assert_eq!(bytes.len(), VestingRecipientAddedEvent::DATA_LEN);
    ...
    #[test]
    fn test_vesting_recipient_added_event_to_bytes() {
        let distribution = Address::new_from_array([1u8; 32]);
        let recipient = Address::new_from_array([2u8; 32]);
        let event = VestingRecipientAddedEvent::new(distribution, recipient, 1000, 0, 100, 200);

        let bytes = event.to_bytes();
        assert_eq!(bytes.len(), EVENT_DISCRIMINATOR_LEN + VestingRecipientAddedEvent::DATA_LEN);
        assert_eq!(&bytes[..8], EVENT_IX_TAG_LE);
        assert_eq!(bytes[8], EventDiscriminators::VestingRecipientAdded as u8);
    }
}
