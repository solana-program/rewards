...
use crate::{
    events::VestingDistributionCreatedEvent,
    state::VestingDistribution,
...
use super::CreateVestingDistribution;

pub fn process_create_vesting_distribution(
    _program_id: &Address,
    accounts: &[AccountView],
    instruction_data: &[u8],
) -> ProgramResult {
    let ix = CreateVestingDistribution::try_from((instruction_data, accounts))?;
    ix.data.validate()?;

    let distribution = VestingDistribution::new(
        ix.data.bump,
        *ix.accounts.authority.address(),
        *ix.accounts.mint.address(),
        *ix.accounts.seeds.address(),
    );
    ...
    let event = VestingDistributionCreatedEvent::new(
        *ix.accounts.authority.address(),
        *ix.accounts.mint.address(),
        *ix.accounts.seeds.address(),
        ix.data.amount,
