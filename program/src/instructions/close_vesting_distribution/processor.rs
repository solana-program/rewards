...
use super::CloseVestingDistribution;

pub fn process_close_vesting_distribution(
    _program_id: &Address,
    accounts: &[AccountView],
    instruction_data: &[u8],
) -> ProgramResult {
    let ix = CloseVestingDistribution::try_from((instruction_data, accounts))?;

    let distribution_data = ix.accounts.distribution.try_borrow()?;
    let distribution = VestingDistribution::from_account(&distribution_data, ix.accounts.distribution, &ID)?;
    ...
    let event = DistributionClosedEvent::new(*ix.accounts.distribution.address());
