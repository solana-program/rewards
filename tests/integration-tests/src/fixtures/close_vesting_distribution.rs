use rewards_program_client::instructions::CloseVestingDistributionBuilder;
use solana_sdk::{
    pubkey::Pubkey,
...
use crate::fixtures::CreateVestingDistributionSetup;
use crate::utils::{find_event_authority_pda, InstructionTestFixture, TestContext, TestInstruction, PROGRAM_ID};

pub struct CloseVestingDistributionSetup {
...
impl CloseVestingDistributionSetup {
    pub fn builder(ctx: &mut TestContext) -> CloseVestingDistributionSetupBuilder<'_> {
        CloseVestingDistributionSetupBuilder::new(ctx)
    }
    ...
    pub fn build_instruction(&self, _ctx: &TestContext) -> TestInstruction {
        ...
        let mut builder = CloseVestingDistributionBuilder::new();
        builder
...
pub struct CloseVestingDistributionSetupBuilder<'a> {
    ctx: &'a mut TestContext,
    token_program: Pubkey,
    amount: u64,
}

impl<'a> CloseVestingDistributionSetupBuilder<'a> {
    ...
    pub fn build(self) -> CloseVestingDistributionSetup {
        let mut distribution_builder = CreateVestingDistributionSetup::builder(self.ctx).amount(self.amount);
...
pub struct CloseVestingDistributionFixture;

impl InstructionTestFixture for CloseVestingDistributionFixture {
    const INSTRUCTION_NAME: &'static str = "CloseVestingDistribution";

    fn build_valid(ctx: &mut TestContext) -> TestInstruction {
        let setup = CloseVestingDistributionSetup::new(ctx);
