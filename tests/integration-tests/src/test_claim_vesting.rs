use solana_sdk::signature::Signer;

use crate::fixtures::{ClaimVestingFixture, ClaimVestingSetup};
use crate::utils::{
    assert_rewards_error, assert_vesting_recipient, test_empty_data, test_missing_signer, test_not_writable,
    test_wrong_current_program, RewardsError, TestContext,
};

#[test]
fn test_claim_vesting_missing_recipient_signer() {
    let mut ctx = TestContext::new();
    test_missing_signer::<ClaimVestingFixture>(&mut ctx, 0, 0);
}

#[test]
fn test_claim_vesting_distribution_not_writable() {
    let mut ctx = TestContext::new();
    test_not_writable::<ClaimVestingFixture>(&mut ctx, 1);
}

#[test]
fn test_claim_vesting_recipient_account_not_writable() {
    let mut ctx = TestContext::new();
    test_not_writable::<ClaimVestingFixture>(&mut ctx, 2);
}

#[test]
fn test_claim_vesting_vault_not_writable() {
    let mut ctx = TestContext::new();
    test_not_writable::<ClaimVestingFixture>(&mut ctx, 4);
}

#[test]
fn test_claim_vesting_recipient_token_account_not_writable() {
    let mut ctx = TestContext::new();
    test_not_writable::<ClaimVestingFixture>(&mut ctx, 5);
}

#[test]
fn test_claim_vesting_wrong_current_program() {
    let mut ctx = TestContext::new();
    test_wrong_current_program::<ClaimVestingFixture>(&mut ctx);
}

#[test]
fn test_claim_vesting_empty_data() {
    let mut ctx = TestContext::new();
    test_empty_data::<ClaimVestingFixture>(&mut ctx);
}

#[test]
fn test_claim_vesting_success_full() {
    let mut ctx = TestContext::new();
    let setup = ClaimVestingSetup::new(&mut ctx);

    let test_ix = setup.build_instruction(&ctx);
    test_ix.send_expect_success(&mut ctx);

    let balance = ctx.get_token_balance(&setup.recipient_token_account);
    assert_eq!(balance, setup.amount);

    assert_vesting_recipient(
        &ctx,
        &setup.recipient_pda,
        &setup.recipient.pubkey(),
        setup.amount,
        setup.amount,
        setup.recipient_bump,
    );
}

#[test]
fn test_claim_vesting_success_token_2022() {
    let mut ctx = TestContext::new();
    let setup = ClaimVestingSetup::new_token_2022(&mut ctx);

    let test_ix = setup.build_instruction(&ctx);
    test_ix.send_expect_success(&mut ctx);

    let balance = ctx.get_token_balance(&setup.recipient_token_account);
    assert_eq!(balance, setup.amount);
}

#[test]
fn test_claim_vesting_partial_25_percent() {
    let mut ctx = TestContext::new();
    let setup = ClaimVestingSetup::builder(&mut ctx).warp_to_end(false).build();

    let duration = setup.end_ts - setup.start_ts;
    let timestamp_25_percent = setup.start_ts + (duration / 4);
    ctx.warp_to_timestamp(timestamp_25_percent);

    let test_ix = setup.build_instruction(&ctx);
    test_ix.send_expect_success(&mut ctx);

    let balance = ctx.get_token_balance(&setup.recipient_token_account);
    let expected_amount = setup.amount / 4;
    assert!(
        balance >= expected_amount - 1 && balance <= expected_amount + 1,
        "Expected ~{}, got {}",
        expected_amount,
        balance
    );
}

#[test]
fn test_claim_vesting_partial_50_percent() {
    let mut ctx = TestContext::new();
    let setup = ClaimVestingSetup::builder(&mut ctx).warp_to_end(false).build();

    let duration = setup.end_ts - setup.start_ts;
    let timestamp_50_percent = setup.start_ts + (duration / 2);
    ctx.warp_to_timestamp(timestamp_50_percent);

    let test_ix = setup.build_instruction(&ctx);
    test_ix.send_expect_success(&mut ctx);

    let balance = ctx.get_token_balance(&setup.recipient_token_account);
    let expected_amount = setup.amount / 2;
    assert!(
        balance >= expected_amount - 1 && balance <= expected_amount + 1,
        "Expected ~{}, got {}",
        expected_amount,
        balance
    );
}

#[test]
fn test_claim_vesting_multiple_claims() {
    let mut ctx = TestContext::new();
    let setup = ClaimVestingSetup::builder(&mut ctx).warp_to_end(false).build();

    let duration = setup.end_ts - setup.start_ts;

    let timestamp_25_percent = setup.start_ts + (duration / 4);
    ctx.warp_to_timestamp(timestamp_25_percent);

    let test_ix1 = setup.build_instruction(&ctx);
    test_ix1.send_expect_success(&mut ctx);

    let balance_after_first = ctx.get_token_balance(&setup.recipient_token_account);
    let expected_first = setup.amount / 4;
    assert!(
        balance_after_first >= expected_first - 1 && balance_after_first <= expected_first + 1,
        "Expected ~{}, got {}",
        expected_first,
        balance_after_first
    );

    let timestamp_50_percent = setup.start_ts + (duration / 2);
    ctx.warp_to_timestamp(timestamp_50_percent);

    let test_ix2 = setup.build_instruction(&ctx);
    test_ix2.send_expect_success(&mut ctx);

    let balance_after_second = ctx.get_token_balance(&setup.recipient_token_account);
    let expected_total = setup.amount / 2;
    assert!(
        balance_after_second >= expected_total - 1 && balance_after_second <= expected_total + 1,
        "Expected ~{}, got {}",
        expected_total,
        balance_after_second
    );
}

#[test]
fn test_claim_vesting_nothing_before_start() {
    let mut ctx = TestContext::new();
    let current_ts = ctx.get_current_timestamp();

    let setup = ClaimVestingSetup::builder(&mut ctx)
        .start_ts(current_ts + 1000)
        .end_ts(current_ts + 2000)
        .warp_to_end(false)
        .build();

    let test_ix = setup.build_instruction(&ctx);
    let error = test_ix.send_expect_error(&mut ctx);

    assert_rewards_error(error, RewardsError::NothingToClaim);
}

#[test]
fn test_claim_vesting_nothing_already_claimed() {
    let mut ctx = TestContext::new();
    let setup = ClaimVestingSetup::new(&mut ctx);

    let test_ix1 = setup.build_instruction(&ctx);
    test_ix1.send_expect_success(&mut ctx);

    ctx.advance_slot();

    let test_ix2 = setup.build_instruction(&ctx);
    let error = test_ix2.send_expect_error(&mut ctx);

    assert_rewards_error(error, RewardsError::NothingToClaim);
}

#[test]
fn test_claim_vesting_unauthorized() {
    let mut ctx = TestContext::new();
    let setup = ClaimVestingSetup::new(&mut ctx);

    let wrong_signer = ctx.create_funded_keypair();
    let wrong_signer_token_account = ctx.create_token_account(&wrong_signer.pubkey(), &setup.mint);

    let test_ix = setup.build_instruction_with_wrong_signer(&ctx, &wrong_signer, wrong_signer_token_account);
    let error = test_ix.send_expect_error(&mut ctx);

    assert_rewards_error(error, RewardsError::UnauthorizedRecipient);
}
